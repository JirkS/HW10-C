...
_Bool pq_push(void *_pq, int label, int cost)
{
   ...
   pq_heap_s *pq = (pq_heap_s *)_pq;
   if (
       pq && pq->len < pq->size && label >= 0 && label < pq->size)
   {
...
_Bool pq_update(void *_pq, int label, int cost)
{
   ...
   if (pq && (label >= 0) && (label < pq->size) && (pq->heapIDX[label] != -1))
   {
      int index = pq->heapIDX[label];
      int old_cost = pq->cost[index];
      pq->cost[index] = cost;
      ...
      if (cost < old_cost)
      {
         while (index > 0)
         {
            ...
            if (pq->cost[index] < pq->cost[parent])
            {
      ...
      else if (cost > old_cost)
      {
         while (true)
         {
            ...
            if (left_child < pq->len && pq->cost[left_child] < pq->cost[smallest])
            {
               smallest = left_child;
            }
            if (right_child < pq->len && pq->cost[right_child] < pq->cost[smallest])
            {
